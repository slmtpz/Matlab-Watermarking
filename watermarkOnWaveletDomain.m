...
S=audioBytes.';
H1=(S(2:2:len)-S(1:2:len))/2;
L1=(S(2:2:len)+S(1:2:len))/2;
H2=(L1(2:2:len/2)-L1(1:2:len/2))/2;
L2=(L1(2:2:len/2)+L1(1:2:len/2))/2;
audioBytes=[L2 H2 H1];
...
% inverse 2-level DWT
L2=audioBytes(1:len/4);
H2=audioBytes(len/4+1:len/2);
H1=audioBytes(len/2+1:len);
L1(2:2:len/2)=L2(1:len/4)+H2(1:len/4);
L1(1:2:len/2)=L2(1:len/4)-H2(1:len/4);
L1 = uint8(L1);
Sp(2:2:len)=L1(1:len/2)+H1(1:len/2);
Sp(1:2:len)=L1(1:len/2)-H1(1:len/2);
Sp = uint8(Sp);

SNR_WATERMARKED = Sp.';

%ab = audioplayer(Sp,Fs);
%play(ab);

%% 1.3) Watermark Extraction

% 2-level DWT
H1=(Sp(2:2:len)-Sp(1:2:len))/2;
L1=(Sp(2:2:len)+Sp(1:2:len))/2;
H2=(L1(2:2:len/2)-L1(1:2:len/2))/2;
L2=(L1(2:2:len/2)+L1(1:2:len/2))/2;
audioBytes=[L2 H2 H1];
